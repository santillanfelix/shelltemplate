...
loadCuentas(){
$ORACLE_HOME/bin/sqlldr	 $usCost/$passCost@$ORACLE_SID control=/opt/batch/credit/Shells/loaderCost.txt<<EOF
EOF
codSt=$?
if [ ! $codSt = 0 ];then
...
loadEquipos(){
$ORACLE_HOME/bin/sqlldr	 $usCost/$passCost@$ORACLE_SID control=/opt/batch/credit/Shells/loaderCostEqs.txt<<EOF
EOF
codSt=$?
if [ ! $codSt = 0 ];then
echo "ERROR EN EL REPORTE" $1
